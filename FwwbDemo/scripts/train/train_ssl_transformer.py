...
NUM_WORKERS = 4   # 数据加载并行数，根据CPU核心数调整
...
def require_tabulars3l_runtime():
    ...
    try:
        ...
        raise RuntimeError(
            "Missing runtime dependencies for TabularS3L transformer integration.\n"
            f"{source_hint}\n"
            "Please make sure your IDE environment has at least:\n"
            "  pip install torch torchvision torchaudio\n"
            "  pip install pytorch-lightning torchmetrics\n"
            "If needed, install the local repo with:\n"
            "  pip install -e .\n"
...
def extract_embeddings_from_dae(torch, data_loader_cls, dataset, dae_lightning_module, batch_size: int = 512, num_workers: int = 4) -> np.ndarray:
    loader = data_loader_cls(dataset, batch_size=batch_size, shuffle=False, num_workers=num_workers, pin_memory=True)
    embeddings = []
...
def detect_outliers_by_distance(embeddings: np.ndarray, percentile: float) -> tuple[np.ndarray, np.ndarray, float]:
...
def save_cluster_outputs(
    ...
    student_cluster_df["risk_label_next_term_numeric"] = pd.to_numeric(student_cluster_df["risk_label_next_term"], errors="coerce")
    student_cluster_df.to_csv(OUTPUT_DIR / "transformer_student_last_semester_clusters.csv", index=False, encoding="utf-8-sig")
    ...
    cluster_summary.to_csv(OUTPUT_DIR / "transformer_student_cluster_summary.csv", index=False, encoding="utf-8-sig")
    ...
    inlier_cluster_summary.to_csv(OUTPUT_DIR / "transformer_inlier_cluster_summary.csv", index=False, encoding="utf-8-sig")

    pd.DataFrame(k_search).to_csv(OUTPUT_DIR / "transformer_cluster_k_search.csv", index=False, encoding="utf-8-sig")

    pca = PCA(n_components=2, random_state=RANDOM_STATE)
    reduced = pca.fit_transform(scaled_embeddings)
    pca_df = student_cluster_df[["student_id", "school_year", "semester", "cluster", "is_outlier", "risk_event_current", "risk_label_next_term_numeric", "embedding_distance_to_center"]].copy()
    pca_df["pca_1"] = reduced[:, 0]
    pca_df["pca_2"] = reduced[:, 1]
    pca_df.to_csv(OUTPUT_DIR / "transformer_student_cluster_pca_coords.csv", index=False, encoding="utf-8-sig")
    ...
    plt.xlabel("PCA-1")
    plt.ylabel("PCA-2")
    plt.title("TabularS3L Transformer DAE Student Clusters (Full)")
    handles, _ = scatter.legend_elements()
    plt.legend(handles, cluster_labels_for_legend, title="Cluster", loc="best")
    plt.tight_layout()
    plt.savefig(OUTPUT_DIR / "transformer_student_cluster_pca.png", dpi=180)
    plt.close()
    ...
    plt.xlabel("PCA-1")
    plt.ylabel("PCA-2")
    plt.title("TabularS3L Transformer DAE Student Clusters (Zoomed)")
    handles, _ = scatter.legend_elements()
    plt.legend(handles, cluster_labels_for_legend, title="Cluster", loc="best")
    plt.tight_layout()
    plt.savefig(OUTPUT_DIR / "transformer_student_cluster_pca_zoom.png", dpi=180)
    plt.close()
    ...
    plt.xlabel("PCA-1")
    plt.ylabel("PCA-2")
    plt.title("Transformer PCA Space Colored by Next-Term Risk")
    plt.tight_layout()
    plt.savefig(OUTPUT_DIR / "transformer_student_risk_pca.png", dpi=180)
    plt.close()

    plt.figure(figsize=(10, 7))
    plt.scatter(reduced[:, 0], reduced[:, 1], c="#4c78a8", s=18, alpha=0.45, label="Inlier")
    if outlier_mask.any():
        plt.scatter(reduced[outlier_mask, 0], reduced[outlier_mask, 1], c="#e45756", s=42, alpha=0.95, marker="x", label="Outlier")
    plt.xlabel("PCA-1")
    plt.ylabel("PCA-2")
    plt.title("Transformer PCA Space with Outlier Highlight")
    plt.legend(loc="best")
    plt.tight_layout()
    plt.savefig(OUTPUT_DIR / "transformer_outlier_pca.png", dpi=180)
    plt.close()
    ...
    if len(scaled_embeddings) >= 200:
        ...
        tsne_df = student_cluster_df[["student_id", "school_year", "semester", "cluster", "is_outlier", "risk_event_current", "risk_label_next_term_numeric", "embedding_distance_to_center"]].copy()
        tsne_df["tsne_1"] = tsne_reduced[:, 0]
        tsne_df["tsne_2"] = tsne_reduced[:, 1]
        tsne_df.to_csv(OUTPUT_DIR / "transformer_student_cluster_tsne_coords.csv", index=False, encoding="utf-8-sig")

        plt.figure(figsize=(10, 7))
        scatter = plt.scatter(tsne_reduced[:, 0], tsne_reduced[:, 1], c=cluster_color_codes, s=18, alpha=0.7, cmap="tab10")
        plt.xlabel("t-SNE-1")
        plt.ylabel("t-SNE-2")
        plt.title("TabularS3L Transformer DAE Student Clusters (t-SNE)")
        handles, _ = scatter.legend_elements()
        plt.legend(handles, cluster_labels_for_legend, title="Cluster", loc="best")
        plt.tight_layout()
        plt.savefig(OUTPUT_DIR / "transformer_student_cluster_tsne.png", dpi=180)
        plt.close()
    ...
    outlier_df.head(50).to_csv(OUTPUT_DIR / "transformer_top_outliers.csv", index=False, encoding="utf-8-sig")
...
def main() -> None:
    ...
    backbone_config = TransformerBackboneConfig(
        ...
        encoder_depth=TRANSFORMER_ENCODER_DEPTH,
    ...
    train_ds = DAEDataset(X=ssl_train_df, continuous_cols=numeric_cols, category_cols=categorical_cols)
    valid_ds = DAEDataset(X=ssl_valid_df, continuous_cols=numeric_cols, category_cols=categorical_cols)
    ...
    ssl_dataset = DAEDataset(X=ssl_features, continuous_cols=numeric_cols, category_cols=categorical_cols)
    last_dataset = DAEDataset(X=last_features, continuous_cols=numeric_cols, category_cols=categorical_cols)

    semester_embeddings = extract_embeddings_from_dae(torch, DataLoader, ssl_dataset, dae_model, num_workers=NUM_WORKERS)
    last_embeddings = extract_embeddings_from_dae(torch, DataLoader, last_dataset, dae_model, num_workers=NUM_WORKERS)
    ...
    semester_embedding_df.to_csv(OUTPUT_DIR / "transformer_semester_embeddings.csv", index=False, encoding="utf-8-sig")

    outlier_mask, distance_to_center, outlier_threshold = detect_outliers_by_distance(last_embeddings_scaled, OUTLIER_DISTANCE_PERCENTILE)
    ...
    summary = {
        ...
        "method": "local TabularS3L Transformer DAE first-phase embedding + KMeans",
        "embedding_type": "FTEmbeddingConfig(required_token_dim=2)",
        "backbone_type": "TransformerBackboneConfig",
        "input_dim": input_dim,
    ...
    (OUTPUT_DIR / "transformer_run_summary.json").write_text(
        json.dumps(summary, ensure_ascii=False, indent=2),
        encoding="utf-8",
    )

    print("Local TabularS3L Transformer DAE pipeline finished.")
